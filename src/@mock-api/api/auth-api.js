...
mock.onGet("/api/auth/sign-in").reply(async (config) => {
  ...
  const isEmailExists = response.data.find((user) =>
    user.emails.some((mail) => mail.email === email)
  );
  const user = isEmailExists;
  ...
  if (error.length === 0) {
    ...
    const access_token = generateJWTToken({ id: user.userId });
...
mock.onGet("/api/auth/access-token").reply(async (config) => {
  ...
  if (verifyJWTToken(access_token)) {
    ...
    const user = res.data.find((user) => user.userId === id);

    // const user = _.cloneDeep(usersApi.find((_user) => _user.userId === id));

    // delete user.password;

    const updatedAccessToken = generateJWTToken({ id: user.userId });
...
mock.onPost("/api/auth/sign-up").reply(async (request) => {
  ...
  if (error.length === 0) {
    const random = FuseUtils.generateGUID()
    const newUser = {
      id: random,
      userId: random,
      from: "custom-db",
...
mock.onPost("/api/auth/user/update").reply(async (config) => {
  const data = JSON.parse(config.data);
  const { user } = data;
