...
function AboutTab({ setEdit, memberData, id }) {
  const me = useSelector(selectUser);

  const user = memberData ;
  const depts = useSelector(state => selectDepartments(state))
  const units = useSelector(state => selectUnits(state))
  const userDepartments  = me.departments.map(id => depts.find(dep => id === dep._id))
  const userUnits = me.units.map(id => units.find(unit => id === unit._id))

  const idForCompare = useSelector(selectUser);
 ...
 const editable = user?._id === idForCompare?._id;
