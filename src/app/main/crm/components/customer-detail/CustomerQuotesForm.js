...
export default function CustomerQuotesForm() {
  const navigate = useNavigate();
  const dispatch = useDispatch();
  const customers = useSelector(selectCustomers);
  const params = useParams();

  const customer = useSelector((state) => selectCustomerById(state, params.id));
  const defaultValues = {
    title: '',
    attention: '',
  ...
  const onSubmit = (formValues) => {
    dispatch(
      ...
      navigate(`/crm/customers/${params.id}`);
    });
  };
